...
$columns = array('nrodoc','pf1', 'codclien', 'nomcli', 'pf2', 'totdoc','iva10', 'fecfac', 'tipdoc');

$query = "SELECT * FROM ventasbk WHERE ";
...
if(isset($_POST["search"]["value"]))
{
 $query .= '
  ...
  OR codclien LIKE "%'.$_POST["search"]["value"].'%" 
  OR nomcli LIKE "%'.$_POST["search"]["value"].'%" 
  OR pf1 LIKE "%'.$_POST["search"]["value"].'%"
  OR pf2 LIKE "%'.$_POST["search"]["value"].'%")
 ';
}
...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array = array();
 $sub_array[] = $wpf1.$row["pf1"].'-'.$wpf2.$row["pf2"].'-'.$row["nrodoc"];
  $sub_array[] = $row["codclien"];
 $sub_array[] = $row["nomcli"];
 $sub_array[] = $row["ruc"];
 $sub_array[] = $nroFormat;
 $sub_array[] = $nroIva;
  $sub_array[] = $total;
